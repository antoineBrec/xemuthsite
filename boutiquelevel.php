...
<div class="left-box">
	...
	<?php   $sql3 =$bdd->query('SELECT name FROM characters WHERE account=\''.$_SESSION['userid'].'\';') or die(mysql_error());	
			$pigeon;
			while($char=$sql3->fetch())
			{	
			echo '<form action="boutiquelevel.php" method="post"><div align="left">'.$char['name'].'' ;	
			...
			$cpt=60;
			$idlevel=$level['level'];
			echo '<v2><select id="niveau" name="level">';
			$cout=0;
				while($cpt != $level['level'])
				{
					...
					echo ('<option value=\''.$cout.'\'>level '.$idlevel.'  prix :'.$cout.'</option>') ;	
				}
				if($cpt==60)
				{
					echo ('<v2><option value=\''.$idlevel.'\'>Vous êtes level Max</option></v2></form>') ;
				}
				else{echo ('<input onclick="boutiquelevel.php" type="submit" name='.$char['name'].' value=\'Acheter pour '.$char['name'].'\'></form>');
				
				if(!empty($_POST[$char['name']])) // Si le formulaire est envoyé.          
					{  
					
						
						
						if($pboutique<$_POST['level'])
							{
								echo ('ta pas les points lol, envois la money, Redirection en cours');
								echo ('<meta http-equiv="refresh" content="3; URL=boutique1.php">');
							}	
						else
						{
								$finale=$pboutique-$_POST['level'];
							 $bdd2->query('UPDATE account SET pboutique=\''.$finale.'\' WHERE username=\''.$_SESSION['username'].'\';');
							 echo ('ta la money gg');
							 echo ('<meta http-equiv="refresh" content="3; URL=boutique1.php">');
						}
